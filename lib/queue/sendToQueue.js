...
const sendToQueue = async (type, payload, options = {}) => {
  ...
  try {
    ...
    if (getCurrentQueue.rows.length > 0 && new Date(`${getCurrentQueue.rows[0].execute_at}Z`).getTime() >= new Date().getTime()) {
    //if (getCurrentQueue.rows.length > 0 && new Date(new Date(getCurrentQueue.rows[0].execute_at).toISOString()).getTime() > new Date().getTime()) {
