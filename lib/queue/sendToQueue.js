...
const sendToQueue = async (type, payload, options = {}) => {
  ...
  try {
    ...
    if (getCurrentQueue.rows.length > 0 && new Date(new Date(getCurrentQueue.rows[0].execute_at).toISOString()).getTime() > new Date(new Date().toISOString()).getTime()) {

      // 72 Sekunden Abstand zwischen den Emails = 50 Emails pro Stunde
      newExecuteAt = new Date(new Date(getCurrentQueue.rows[0].execute_at.toISOString()).setSeconds(new Date(getCurrentQueue.rows[0].execute_at.toISOString()).getSeconds() + 72))
    }
