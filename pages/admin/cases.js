...
export default function Home() {
  ...
  const changeMessageStatus = async (messageId, newStatus) => {
    ...
    await fetch(`/api/admin/message?messageId=${messageId}`, {method: 'PATCH', 
    ...
    <>
      <AdminWrapper>
        ...
        <div className="container mt-1 mb-3">
          ...
          <div className="row mt-0">
            ...
            <div className="col-8 fw-bold text-end">
              <div className="btn-group" role="group">
                <button className={`btn btn-light${status === 'OPEN' ? ' active' : ''}`} onClick={() => setStatus('OPEN')}>Offen ({messages.filter(m => m.status === 'OPEN')?.length || 0})</button>
                <button className={`btn btn-light${status === 'CLOSED' ? ' active' : ''}`} onClick={() => setStatus('CLOSED')}>Geschlossen ({messages.filter(m => m.status === 'CLOSED')?.length || 0})</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container mb-3">
          ...
          {messages.filter(m => m.message_type === 'case' && m.status === status && m.zipcode.startsWith(formZipcode)).map(message => <React.Fragment key={message.message_id}>
          <div className="row">
            <div className="col-12">
                  ...
                  <div className="col-1 text-end">
                    <div className="dropdown">
                      ...
                      <ul className="dropdown-menu">
                        <li>
                          {status === 'OPEN' ? <div className="dropdown-item cursor-pointer" title="Schließen" onClick={() => changeMessageStatus(message.message_id, 'CLOSED')}>Schließen</div> : null}
                          {status === 'CLOSED' ? <div className="dropdown-item cursor-pointer" title="Öffnen"onClick={() => changeMessageStatus(message.message_id, 'OPEN')}>Öffnen</div> : null}
                        </li>
                        <li>
                          <div className="dropdown-item cursor-pointer" title="Löschen" onClick={() => deleteMessage(message.message_id)}>Löschen</div>
                        </li>
                      </ul>
                    </div>
                  </div>
