...
export default function Home() {
  ...
  const [selectedPeriodDropdownText, setSelectedPeriodDropdownText] =
    useState("Letzten 30 Tage");
  const [selectedComparisonText, setSelectedComparisonText] = useState(
    "seit letzten 30 Tage",
  );

  const { data: audits, error: auditError } = useSWR(
    `/api/admin/audits?timePeriod=${selectedPeriod}`,
  ...
  const addStatisticCard = (whichCard) => {
    const cardData = {
      SIGNUPS: {
        ...
        color: "bg-success",
        value:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "addAccount",
          ).length || 0,
        difference:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "addAccount",
          ).length -
            audits?.lastAuditPeriod?.filter(
              (audit) => audit.action_type === "addAccount",
            ).length || 0,
      },
      DELETIONS: {
        ...
        value:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "deleteAccount",
          ).length || 0,
        difference:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "deleteAccount",
      ...
      SEARCHES: {
        ...
        value:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "addCaseHelpRequest",
          ).length || 0,
        difference:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "addCaseHelpRequest",
      ...
      OFFERS: {
        ...
        value:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "addCaseHelpOffer",
          ).length || 0,
        difference:
          audits?.currentAuditPeriod?.filter(
            (audit) => audit.action_type === "addCaseHelpOffer",
        ...
        <div className="card card-stats mb-4 mb-xl-0">
          <div className="card-body">
            <div className="row">
              <div className="col-8">
                <h5 className="card-title text-uppercase text-muted mb-0">
                  {cardData[whichCard].title}
                </h5>
                <span className="h2 font-weight-bold mb-0">
                  {cardData[whichCard].value}
                </span>
              </div>
              <div className="col-4 text-end">
                <div
                  className={`icon icon-shape text-white rounded-circle shadow ${cardData[whichCard].color}`}
                >
            ...
            <p className="mt-3 mb-0 text-muted text-md">
              {cardData[whichCard].difference < 0 ? (
                <span className="text-danger me-2">
                  <i className="fa fa-arrow-down"></i>{" "}
                  {cardData[whichCard].difference}
                </span>
              ) : null}
              {cardData[whichCard].difference > 0 ? (
                ...
                <ul className="dropdown-menu">
                  <li>
                    <a
                      ...
                      href="#"
                      onClick={() => selectNewPeriod("LAST24H")}
                    >
                      Letzten 24 Stunden
                    </a>
                  </li>
                  <li>
                    <a
                      className="dropdown-item"
                      href="#"
                      onClick={() => selectNewPeriod("LAST7D")}
                    >
                      Letzten 7 Tage
                    </a>
                  </li>
                  <li>
                    <a
                      className="dropdown-item"
                      href="#"
                      onClick={() => selectNewPeriod("LAST30D")}
                    >
                      Letzten 30 Tage
                    </a>
                  </li>
                  <li>
                    <a
                      className="dropdown-item"
                      href="#"
                      onClick={() => selectNewPeriod("LAST365D")}
                    >
                      Letzten 365 Tage
                    </a>
                  </li>
