...
export default function AdminCaseAdd({ query }) {
  ...
  const { data: message, error: messageError } = useSWR(
    `/api/admin/case?caseId=${query.caseId}`,
  ...
  const [formExperienceWithAnimalOther, setFormExperienceWithAnimalOther] =
    useState("");
  ...
  useEffect(() => {
    if (focusOut && formStreet && formStreetNumber && formZipcode && formCity) {
      searchAddress(
        `${formStreet},${formStreetNumber},${formZipcode},${formCity}`,
      );
    } else {
      setFocusOut(false);
    }
  ...
  const searchAddress = async (value) => {
    const location = await (
      await fetch(
        `https://nominatim.openstreetmap.org/search?q=${value}?format=json&addressdetails=1&linkedplaces=1&namedetails=1&limit=5&email=info@silberpfoten.de`,
      )
    ).json();
  ...
  if ((!session && !sessionError) || (!message && !messageError))
    return <Loading />;
  ...
  const handleSubmit = async (e) => {
    ...
    const patchRequest = await fetch(
      `/api/admin/message?messageId=${message.message_id}`,
      {
        method: "PATCH",
        headers: {
    ...
    if (patchRequest.status === 200) {
      ...
      router.push(
        `/admin/case/${query.caseId}/${slugify(message.subject, { lower: true })}`,
      );
  ...
  const deleteMedia = async (mediaId) => {
    ...
    if (confirmed) {
      const deleteMediaRequest = await fetch(
        `/api/admin/message/media?mediaId=${mediaId}`,
        { method: "DELETE" },
      );
            ...
            <div className="row">
              ...
              <div className="col-6">
                <div className="btn-group float-end" role="group">
                  <Link href="/admin/cases" className="btn btn-secondary">
                    Abbrechen
                  </Link>
            ...
            <div className="row mt-3">
              <div className="col-12">
                    ...
                    <div className="mb-2 bg-danger rounded text-white p-2">
                      <div className="fw-bold">
                        ...
                        <span className="fw-normal">
                          (Hochgeladen: {filesize(calculateUploadedTotalSize())}
                          )
                        </span>
                      </div>
                      <div className="">
                        Die hochgeladenen Dateien überschreiten das Limit von 10
                        MB.
                      </div>
                    </div>
                  </>
                ...
                {message.has_media?.length > 0 ? (
                  <>
                    {message.has_media?.map((media) => (
                      <React.Fragment key={media.message_has_media_id}>
                        <div className="border mb-1 p-1 rounded">
                          <div className="row align-items-center">
                            <div className="col-2">
                              {["png", "jpeg", "gif"].includes(
                                media.mimetype,
                              ) ? (
                                <Image
                                  src={`data:image/webp;base64,${media.thumbnail}`}
                            ...
                            <div className="col-2 text-center">
                              <i
                                ...
                                onClick={() =>
                                  deleteMedia(media.message_has_media_id)
                                }
                                style={{ fontSize: 14 }}
                              ></i>
                            </div>
                            ...
                            <div className="col-2">
                              {[
                                ...
                                <div
                                  ...
                                  style={{
                                    backgroundImage: `url(${upload.base64})`,
            ...
            <div className="row mt-1">
              ...
              <div className="col-12 col-md-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    setFormStreetNumber(e.target.value);
                    resetCoords();
                  }}
                  onBlur={() => setFocusOut(true)}
                  required
            ...
            {formAutoCompleteValues ? (
              <div className="border rounded-bottom p-2 bg-light mb-3">
                {formAutoCompleteValues?.length > 0 ? (
                  <>
                    {!placeId ? (
                      <div className="p-1 text-danger fw-bold">
                        <i className="bi bi-info-circle-fill"></i>{" "}
                        <small>Wähle die passendste Adresse aus:</small>
                      </div>
                    ) : (
                      <div className="p-1 text-success">
                        <i className="bi bi-info-circle-fill"></i>{" "}
                        <small>Wähle die passendste Adresse aus:</small>
                      </div>
                    )}
                    ...
                    {formAutoCompleteValues.map((item) => (
                      <div
                        key={item.place_id}
                        className={`p-1 cursor-pointer ${placeId && placeId != item.place_id ? "text-muted" : null}`}
                        onClick={(e) => {
                          setPlaceId(`${item.place_id}`);
                          setCoordLat(`${item.lat}`);
                          setCoordLon(`${item.lon}`);
                        }}
                      >
            ...
            <div className="row mt-3">
              <div className="col-12 col-md-4">
                ...
                <input
                  ...
                  className="form-control"
                  placeholder="3"
                  value={formSearchRadius}
                  onChange={(e) => setFormSearchRadius(e.target.value)}
                  required
                />
              </div>
            </div>
            ...
            <div className="row mt-2">
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    className="row cursor-pointer align-items-center"
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("go_walk")
                          ? [...formSupportingActivity, ...["go_walk"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "go_walk",
                            ),
                      )
                    }
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("veterinary_trips")
                          ? [...formSupportingActivity, ...["veterinary_trips"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "veterinary_trips",
                            ),
                      )
                    }
                  >
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("animal_care")
                          ? [...formSupportingActivity, ...["animal_care"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "animal_care",
                            ),
                      )
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("events")
                          ? [...formSupportingActivity, ...["events"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "events",
                            ),
                      )
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("baking_cooking")
                          ? [...formSupportingActivity, ...["baking_cooking"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "baking_cooking",
                            ),
                      )
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("creative_workshop")
                          ? [
                              ...formSupportingActivity,
                              ...["creative_workshop"],
                            ]
                          : formSupportingActivity.filter(
                              (i) => i !== "creative_workshop",
                            ),
                      )
                    }
                  >
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("public_relation")
                          ? [...formSupportingActivity, ...["public_relation"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "public_relation",
                            ),
                      )
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("light_office_work")
                          ? [
                              ...formSupportingActivity,
                              ...["light_office_work"],
                            ]
                          : formSupportingActivity.filter(
                              (i) => i !== "light_office_work",
                            ),
                      )
                    }
                  >
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormSupportingActivity(
                        !formSupportingActivity.includes("graphic_work")
                          ? [...formSupportingActivity, ...["graphic_work"]]
                          : formSupportingActivity.filter(
                              (i) => i !== "graphic_work",
                            ),
                      )
            ...
            <div className="row justify-content-center mt-4">
              <div className="col-12">
                <span className="p small ms-1">
                  Benötigte Erfahrungen mit Tierarten
                </span>
              </div>
            </div>
            <div className="row mt-2">
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    className="row cursor-pointer align-items-center"
                    onClick={() =>
                      setFormExperienceWithAnimal(
                        !formExperienceWithAnimal.includes("dog")
                          ? [...formExperienceWithAnimal, ...["dog"]]
                          : formExperienceWithAnimal.filter((i) => i !== "dog"),
                      )
                    }
                  >
                    <div className="col-2 text-center">
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormExperienceWithAnimal(
                        !formExperienceWithAnimal.includes("cat")
                          ? [...formExperienceWithAnimal, ...["cat"]]
                          : formExperienceWithAnimal.filter((i) => i !== "cat"),
                      )
                    }
                  >
                    <div className="col-2 text-center">
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormExperienceWithAnimal(
                        !formExperienceWithAnimal.includes("small_animal")
                          ? [...formExperienceWithAnimal, ...["small_animal"]]
                          : formExperienceWithAnimal.filter(
                              (i) => i !== "small_animal",
                            ),
                      )
                    }
              ...
              <div className="col-12">
                <div className="bg-light rounded p-2">
                  <div
                    ...
                    onClick={() =>
                      setFormExperienceWithAnimal(
                        !formExperienceWithAnimal.includes("bird")
                          ? [...formExperienceWithAnimal, ...["bird"]]
                          : formExperienceWithAnimal.filter(
                              (i) => i !== "bird",
                            ),
                      )
                    ...
                    onClick={() =>
                      setFormExperienceWithAnimal(
                        !formExperienceWithAnimal.includes("other")
                          ? [...formExperienceWithAnimal, ...["other"]]
                          : formExperienceWithAnimal.filter(
                              (i) => i !== "other",
                            ),
                      )
                    ...
                    <>
                      <div className="row cursor-pointer align-items-center">
                        <div className="col-12">
                          <input
                            type="text"
                            name="formExperienceWithAnimalOther"
                            className="form-control mt-2"
                            placeholder="Tierart"
                            value={formExperienceWithAnimalOther}
                            onChange={(e) =>
                              setFormExperienceWithAnimalOther(e.target.value)
                            }
                            required
                          />
                        </div>
