...
export default function AdminCaseAdd() {
  ...
  useEffect(() => {
    const findLocation = async () => {
      const location = await (
        await fetch(
          `https://nominatim.openstreetmap.org/search?postalcode=${formZipcode}&country=germany&format=json&addressdetails=1&linkedplaces=1&namedetails=1&limit=1&email=info@silberpfoten.de`,
        )
      ).json();
      ...
      for (const user of users?.filter(
        (u) =>
          u.newsletter &&
          !u.newsletter_deactivated &&
          !u.newsletter_bounced &&
          u.status === "USER",
      )) {
    ...
    if (
      formZipcode.length === 5 &&
      allDistancesOfUsers.length === 0 &&
      formSearchRadius > 0
    ) {
      findLocation();
  ...
  const handleSubmit = async (e) => {
    ...
    if (putRequest.status === 200) {
      ...
      router.push(
        `/admin/case/${data.body.caseId}/${slugify(data.body.subject, { lower: true })}`,
      );
  ...
  return (
    <>
      <AdminWrapper>
        <NavigationHeader goBack="/admin/cases" title="Suchaufträge" />
            ...
            <div className="row">
              ...
              <div className="col-6">
                <div className="btn-group float-end" role="group">
                  ...
                  allDistancesOfUsers?.filter((d) => d <= formSearchRadius)
                    ?.length > 500 ? (
                    <button
                        ...
                        <div className="mb-2 bg-danger rounded text-white p-2">
                          <div className="fw-bold">
                            ...
                            <span className="fw-normal">
                              (Hochgeladen:{" "}
                              {filesize(calculateUploadedTotalSize())})
                            </span>
                          </div>
                          <div className="">
                            Die hochgeladenen Dateien überschreiten das Limit
                            von 10 MB.
                          </div>
                        </div>
                      </>
                          ...
                          <div className="row align-items-center">
                            <div className="col-2">
                              {upload.type === "application/pdf" ? (
                                <div
                                  className="rounded border text-center pt-1 bg-light"
                ...
                <span className="p small ms-1">
                  ...
                  {allDistancesOfUsers?.filter((d) => d <= formSearchRadius)
                    ...
                    <span className="fw-bold text-danger">
                      Maximal 500 User möglich. Bitte Suchradius verkleinern!
                    </span>
                  ) : null}
                </span>
                <input
