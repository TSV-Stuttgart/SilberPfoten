  ...
  try {
    if (request.method === 'DELETE') {
      ...
      const dbRequest = await db.query(
        `DELETE FROM public.accepted_case WHERE user_id = $1 AND message_id = $2 RETURNING user_id, message_id`, 
    ...
    if (request.method === 'POST') {
      ...
      const dbRequest = await db.query(
        `INSERT INTO public.accepted_case (user_id, message_id) VALUES ($1,$2) RETURNING user_id, message_id`, 
        [token.user.user_id, request.query.messageId]
      )
